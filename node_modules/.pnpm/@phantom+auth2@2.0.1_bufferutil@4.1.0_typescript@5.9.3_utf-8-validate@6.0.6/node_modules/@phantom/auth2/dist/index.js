...
__export(src_exports, {
  Auth2KmsRpcClient: () => Auth2KmsRpcClient,
  Auth2Stamper: () => Auth2Stamper,
  Auth2Token: () => Auth2Token,
  Auth2TokenExpiredError: () => Auth2TokenExpiredError,
  _createCodeChallenge: () => _createCodeChallenge,
  _deriveNonce: () => _deriveNonce,
  _getOrCreateAppWallet: () => _getOrCreateAppWallet,
  completeAuth2Exchange: () => completeAuth2Exchange,
  createCodeVerifier: () => createCodeVerifier,
  createConnectStartUrl: () => createConnectStartUrl,
  decodeJwtClaims: () => decodeJwtClaims,
  exchangeAuthCode: () => exchangeAuthCode,
  prepareAuth2Flow: () => prepareAuth2Flow,
  refreshToken: () => refreshToken,
  validateAuth2Callback: () => validateAuth2Callback
});
...
var Auth2KmsRpcClient = class {
  constructor(stamper, options) {
    ...
    axiosInstance.interceptors.request.use(async (config) => {
      ...
      const stamp = await this.stamper.stamp({ data: import_buffer.Buffer.from(requestBody, "utf-8") });
      config.headers["x-phantom-stamp"] = stamp;
      return config;
    });
    const configuration = new import_openapi_wallet_service.Configuration({ basePath: options.apiBaseUrl });
    this.kmsApi = new import_openapi_wallet_service.KMSRPCApi(configuration, options.apiBaseUrl, axiosInstance);
  }
  ...
  async getOrCreatePhantomOrganization(publicKey) {
    return await this.postKmsRpc({
      method: import_openapi_wallet_service.GetOrCreatePhantomOrganizationMethodEnum.getOrCreatePhantomOrganization,
      params: { publicKey },
  ...
  async getOrCreateWalletWithTag(args) {
    return await this.postKmsRpc({
      method: import_openapi_wallet_service.GetOrCreateWalletWithTagMethodEnum.getOrCreateWalletWithTag,
      params: args,
      timestampMs: Date.now()
    });
  }
...
async function createAuth2RequestJar(input) {
  ...
  const encodedHeader = (0, import_base64url2.base64urlEncode)(textEncoder.encode(JSON.stringify(header)));
  const encodedPayload = (0, import_base64url2.base64urlEncode)(textEncoder.encode(JSON.stringify(input.payload)));
  ...
  return `${signingInput}.${(0, import_base64url2.base64urlEncode)(new Uint8Array(signature))}`;
}
...
var DAPP_WALLET_DERIVATIONS = [
  {
    curve: import_openapi_wallet_service2.DerivationInfoCurveEnum.ed25519,
    derivationPath: "m/44'/501'/0'/0'",
    addressFormat: import_openapi_wallet_service2.DerivationInfoAddressFormatEnum.solana
  },
  {
    curve: import_openapi_wallet_service2.DerivationInfoCurveEnum.secp256k1,
    derivationPath: "m/44'/60'/0'/0/0",
    addressFormat: import_openapi_wallet_service2.DerivationInfoAddressFormatEnum.ethereum
  },
  {
    curve: import_openapi_wallet_service2.DerivationInfoCurveEnum.secp256k1,
    derivationPath: "m/84'/0'/0'/0",
    addressFormat: import_openapi_wallet_service2.DerivationInfoAddressFormatEnum.bitcoinSegwit
  },
  {
    curve: import_openapi_wallet_service2.DerivationInfoCurveEnum.ed25519,
    derivationPath: "m/44'/784'/0'/0'/0'",
    addressFormat: import_openapi_wallet_service2.DerivationInfoAddressFormatEnum.sui
  }
];
...
function createCodeVerifier() {
  ...
  return (0, import_base64url3.base64urlEncode)(bytes).slice(0, 96);
}
...
async function _deriveNonce(keyPair, salt) {
  ...
  return (0, import_base64url3.base64urlEncode)(await (0, import_crypto.sha256)(combined));
}
async function _createCodeChallenge(codeVerifier) {
  return (0, import_base64url3.base64urlEncode)(await (0, import_crypto.sha256)(new TextEncoder().encode(codeVerifier)));
}
...
async function _getOrMigrateWallet({
  ...
  const publicKey = (0, import_base64url3.base64urlEncode)(import_bs582.default.decode(keyInfo.publicKey));
